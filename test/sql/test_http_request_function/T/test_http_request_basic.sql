...
-- description: Basic tests for http_request() function with JSON config
...
SELECT http_request(NULL);

-- Test 2: Empty string returns JSON with error
SELECT json_query(http_request(''), '$.status') as status;

-- Test 3: Invalid URL returns JSON with error
SELECT json_query(http_request('not a valid url'), '$.status') as status;

-- Test 4: Simple GET request (1-arg form) - check status code
SELECT json_query(http_request('https://jsonplaceholder.typicode.com/posts/1'), '$.status') as status;

-- Test 5: GET and parse JSON response body (1-arg form)
SELECT json_query(
    json_query(http_request('https://jsonplaceholder.typicode.com/posts/1'), '$.body'),
    '$.id'
) as post_id;
...
SELECT json_query(http_request(
    ...
    '{"method": "POST", "headers": {"Content-Type": "application/json"}, "body": {"text": "Test 16: Simple message from StarRocks http_request function"}}'
), '$.status') as status;
...
SELECT json_query(http_request('https://self-signed.badssl.com/'), '$.status') as status;

-- Test 20: Self-signed certificate with ssl_verify=false - should succeed
SELECT json_query(http_request('https://self-signed.badssl.com/', '{"ssl_verify": false}'), '$.status') as status;

-- Test 21: Expired certificate with ssl_verify=true (default) - should fail
SELECT json_query(http_request('https://expired.badssl.com/'), '$.status') as status;

-- Test 22: Expired certificate with ssl_verify=false - should succeed
SELECT json_query(http_request('https://expired.badssl.com/', '{"ssl_verify": false}'), '$.status') as status;
